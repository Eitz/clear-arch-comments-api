...
describe("comment", () => {
  ...
  it("can have a source browser", () => {
    ...
    expect(makeComment(withBrowser).getSource().getBrowser()).toBe(
      withBrowser.source.browser
    );
  });
  it("can have a source referrer", () => {
    const withRef = makeFakeComment({});
    expect(makeComment(withRef).getSource().getReferrer()).toBe(
      withRef.source.referrer
    );
  });
});
